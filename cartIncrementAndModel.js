    ...
    fetch('./assets/other_resources/books.json')
      ...
      .then(books => {
        ...
        if (book) {
          ...
          const modalBookImage = document.querySelector("#addToCartModal .card-img-top");
  
          // Retrieve the book title, description, and author from the book object
          modalBookCategory.textContent = `${'#'+book.category || 'Unknown category'}`;
          modalBookTitle.textContent = `${book.title || 'Unknown Title'}`;
          modalBookPrice.textContent = `${'$' +book.price || 'Unknown Price'}`;
          modalBookDescription.textContent = `${book.description || 'Unknown Description'}`;
